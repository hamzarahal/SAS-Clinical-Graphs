...
/* jenner-check: display the derived startline/endline/durable columns
   produced by the DATA step above (added only to surface the result) */
proc print data=swimmer;
  var item stage status start end startline endline durable ymin;
run;
